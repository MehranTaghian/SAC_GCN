...
class RobotEnv(gym.GoalEnv):
    ...
    def step(self, action):
        ...
        # modification here
        old_value = self.sim.data.get_joint_qpos("robot0:shoulder_lift_joint").copy()
        # end of modification
        ...
        # modification here
        new_value = self.sim.data.get_joint_qpos("robot0:shoulder_lift_joint").copy()
        delta = new_value - old_value
        # slippery_value = old_value + delta * self.np_random.normal(0, 0.1)
        slippery_value = new_value + (delta != 0) * 0.05
        self.sim.data.set_joint_qpos("robot0:shoulder_lift_joint", slippery_value)
        self.sim.forward()
        # end of modification
